...
| hb-sk skill | `hb-sk-<contract>` | product knowledge skill: one pinned contract, thin `SKILL.md`, SSOT pointer into the docs. Eleven ship in the template: `component-framework`, `surface-framework`, `interface-framework`, `domain-framework`, `contracts`, `tdd`, `test-runner`, `local-runtime`, `cloud`, `git`, `abc`. One owner each except `abc` (parent fallback; Inquisitor loads none) and `surface-framework` (Warrior-owned; Trickster may load for surface tests) ([[HARNESS]]) | copying vendor docs into `skills/*/references/`; a product skill with no owner |
| hb-ag agent | `hb-ag-<area>` | product area-owner agent under `agents/`. Live: `contracts` (The Cleric), `service` (The Dwarf), `surface` (The Warrior), `ops` (The Wizard), `judge` (The Inquisitor), `test` (The Trickster), `git` (The Bard). Stem + title in the body ([[HARNESS]]) | unprefixed `warrior` / `archer` / `cleric` / `trickster` / `bard`; restoring `The Archer` as a live title; restoring `kbot-*` builders |
| The Cleric | `hb-ag-contracts`, title **The Cleric** | sole writer of `docs/INTERFACES.md` and `docs/contracts/`; `Agent` → Dwarf, Warrior, Trickster | `The Archer` as a live title; unprefixed `cleric`; `kwf-archer` (archived *surface* builder of `triage-and-fix`); walking into `{{service tree}}` or `{{surface tree}}` |
| The Dwarf | `hb-ag-service`, title **The Dwarf** | owns `{{service tree}}` only; `Agent` → Cleric (Trickster for tests; Wizard for infra; never Warrior) | writing `INTERFACES.md`, tests, `docs/tdds/`, or the local runtime; `git` / `gh`; `kwf-warrior` (archived *service* builder — inverted title) |
| The Warrior | `hb-ag-surface`, title **The Warrior** | owns `{{surface tree}}` except tests; `Agent` → Cleric (Trickster for tests; Wizard for infra; never Dwarf); the screen in `{{interface language}}`. Optional: a headless project deletes it | `kwf-warrior` (that was the service); `kwf-archer` (that *was* the surface); writing `INTERFACES.md`; Agent The Dwarf; `git` / `gh` |
...
| The Trickster | `hb-ag-test`, title **The Trickster** | owns `docs/tdds/` and all tests; no `Agent`; cannot give face | writing product code or `INTERFACES.md`; unprefixed `trickster`; `git` / `gh` |
| The Bard | `hb-ag-git`, title **The Bard** | the only `hb-ag-*` that may `git` / `gh`; no product-tree Write; no `Agent` | unprefixed `bard`; other agents running `git` or `gh`; writing app code while shipping; `kwf-bard` as this agent |
