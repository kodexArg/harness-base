...
| hb-ag agent | `hb-ag-<area>` | product agent under `agents/`. Live tree party: `contracts` (The Cleric), `service` (The Dwarf), `surface` (The Elf), `ops` (The Wizard), `judge` (The Inquisitor), `test` (The Trickster), `git` (The Bard). Live hunting party: `hunter` (The Hunter), `hawk` (The Hawk), `hound` (The Hound). Stem + title in the body ([[HARNESS]]) | unprefixed `warrior` / `archer` / `elf` / `cleric` / `trickster` / `bard` / `hunter` / `hawk` / `hound`; restoring `The Archer` or `The Warrior` as a live title; restoring `kbot-*` builders |
