...
| hb-ag agent | `hb-ag-<area>` | product area-owner agent under `agents/`. Live: `contracts` (The Cleric), `service` (The Dwarf), `surface` (The Elf), `ops` (The Wizard), `judge` (The Inquisitor), `test` (The Trickster), `git` (The Bard). Stem + title in the body ([[HARNESS]]) | unprefixed `warrior` / `archer` / `elf` / `cleric` / `trickster` / `bard`; restoring `The Archer` or `The Warrior` as a live title; restoring `kbot-*` builders |
...
| The Dwarf | `hb-ag-service`, title **The Dwarf** | owns `{{service tree}}` only; `Agent` → Cleric (Trickster for tests; Wizard for infra; never Elf) | writing `INTERFACES.md`, tests, `docs/tdds/`, or the local runtime; `git` / `gh`; Agent The Elf; `kwf-warrior` (archived *service* builder) |
