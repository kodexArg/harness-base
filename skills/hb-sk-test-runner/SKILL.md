...
description: >
  ...
  parametrize, isolation, a real database, no tautological mocks, and
  live-credential markers deselected in the default slice. Load when
  authoring or debugging service tests, fixtures, shadow tests, or a
  red TDD list — even if the skill is not named. Triggers: test
  runner, parametrize, mock, fixture, failing tests. Owner: The
  Trickster (hb-ag-test).
applies_when:
...
1. Read the TDD entry (`hb-sk-tdd`) and a neighbor test file in the same area.
...
| DB | A real database, not a mocked one. Build rows with fixtures like the neighbor files. |
